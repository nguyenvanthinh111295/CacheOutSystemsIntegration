...
namespace OutSystems.HubEdition.DatabaseProvider.Cache.Introspection {
    ...
    public class CacheIntrospectionService : BaseIntrospectionService {
        ...
        private string ExtractTableNameFromQualifiedName(string qualifiedTableName) {
            int indx = qualifiedTableName.LastIndexOf('.');
            string theTableName = string.Copy(qualifiedTableName);

            if (indx >= 0 && qualifiedTableName.Length > indx) {
                theTableName = qualifiedTableName.Substring(indx + 1);
            }
